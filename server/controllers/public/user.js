...
router.post("/register", async (req, res) => {
  try {
    let { fullName, email, phone, password, age, gender } = req.body;
    gender = gender.toLowerCase();

    let dupUser = await userModel.findOne({ $or: [{ email }, { phone }] });
    if (dupUser) {
    ...
    await sendMail(
      email,
      "Swiggy Verification",
      `visit this link to verifiy your e-mail http://localhost:5000/public/email-verify/:${emailToken}`
    );
    await sendSms(
      phone,
      `visit this link to verify your phone http://localhost:5000/public/phone-verify/:${phoneToken}`
    );
    ...
    await userModel.insertOne(object);
...
router.get("/email-verify/:token", async (req, res) => {
  try {
    ...
    let user = await userModel.findOne({ "isVerifiedToken.emailToken": token });
...
router.get("/phone-verify/:token", async (req, res) => {
  try {
    ...
    let user = await userModel.findOne({ "isVerifiedToken.phoneToken": token });
...
router.post("/setnewpassword", async (req, res) => {
  try {
    ...
    let bPass = await bcrypt.hash(password, 10);
    console.log(bPass);
    await userModel.updateOne({otp},{$set:{password : bPass},$unset : {otp : "" }},{new : true})
